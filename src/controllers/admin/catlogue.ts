...
import { catalogueModel } from "../../database/models/catlogue ";
...
export const add_catlogue = async(req, res)=>{
    ...
    try{
        ...
        const response = await new catalogueModel(body).save()
...
export const edit_catlogue_by_id = async(req, res)=>{
    ...
    try{
        body.updatedBy = ObjectId(user?._id)
        const response = await catalogueModel.findOneAndUpdate({_id:ObjectId(body._id), isDeleted:false}, body, {new:true})
...
export const delete_catlogue_by_id = async(req, res)=>{
    ...
    let {id} = req.params
    try{
        const response = await catalogueModel.findOneAndUpdate({_id:ObjectId(id), isDeleted: false}, {isDeleted: true}, {new:true})
...
export const get_all_catlogue = async(req, res) => {
    ...
    try{
        ...
        response = await catalogueModel.find(match)
        .sort({ createdAt: -1 })
            .skip((page - 1) * limit)
            .limit(limit)
...
export const get_by_id_catlogue = async(req, res)=>{
    ...
    let {id}=req.params
    try{
        const response = await catalogueModel.findOne({_id:ObjectId(id), isDeleted : false})
